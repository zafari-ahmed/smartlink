...
class BookingController extends Controller
{
	...
	public function actionReportall(){
		$phaseId = Yii::app()->session->get('userModel')['phase_id'];
		$list = array (
	        array('Block #','Plot Type','Plot #','Name','CNIC','Mobile #','Booking Date','Total Amout','Paid Amout','Dealer','File Status','Document Completed'),
	    );
        ...
        foreach($bookings as $booking):
			...
			$list[$count][] = ($booking->CPDCount == 3)?'Yes':'No';
			// if($booking->blocked == 2){
   ...
   //          }
	...
	public function actionReportalltransaction(){
		...
		$phaseId = Yii::app()->session->get('userModel')['phase_id'];
		$list = array (
	        array('Receipt No','Client Name','Block #','Plot Type','Plot #','Size','Payment Mode','Total','Comment','Date','Transaction','bank','Reference Number'),
	    );
      		
      	
		$model = new CustomerPlotTransactions;
      	$bookings = $model::model()->findAllBySQL("SELECT * ,SUM(t.amount) as total,GROUP_CONCAT( DISTINCT p.mode ORDER BY p.id SEPARATOR ',') as p_modes FROM `customer_plot_transactions` t LEFT JOIN payment_schedule_payment_modes p ON p.id = t.plot_payment_mode_id LEFT JOIN customer_plots cp ON cp.id = t.plot_id WHERE (t.status = 1 AND cp.status = 1) AND cp.phase_id = $phaseId GROUP BY transaction_number ORDER BY `transaction_number` ASC");
        ...
        foreach($bookings as $tt):
            ...
            $list[$count][] = @$tt->plot->plot->plot_type;
	...
	public function getPlotLedgerDetail($id){
		...
		foreach($paymentmodes as $modes): $rowBalance = 0;
			...
			$var_sum = CustomerPlotTransactions::model()->findBySql('select sum(`amount`) as `total` from customer_plot_transactions WHERE plot_id = :id AND plot_payment_mode_id = :modeid', array(':id'=>$booking->id,':modeid'=>$modes->id));
			$checkValue = false;
			if($modes->mode=='booking'){
				if($modes->amount != $var_sum->total){
					$divideVal = $modes->amount - $var_sum->total;
			    	echo '<b>'.$this->actionGetModeDueDate(@$booking, @$booking->createdOn,strtolower(@$modes->mode),round($checkValue)).'-->'.$modes->mode.'---->'.floor($divideVal).'</b></br/>';break;
				}
			}
			    ...
			    if($checkValue != 36){ 
                	if($netTotalCheck > 0){
                            if($checkValue!=0){
                                ...
                                echo '<b>Paid Upto: '.date('M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValue months")).'</b><br/>';
                            }}
                    if($fractionVal!=0){
                    		echo '<b>Previos Balance: '.number_format((1-$fractionVal) * number_format(($modes->amount/36),'2','.',''),'2','.','').'<b></br>';
                    }
                    $checkValueMonthly = ($fractionVal!=0)?$checkValue+1:$checkValue;
                    echo '<b>Due Months '.$this->getDateDiff(date('d M, Y'),date('d M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValueMonthly months"))).'</b></br>';
                    ...
                    if($fractionVal!=0){
                        ...
                        echo '<b>Total: '.number_format($dueTotal,'2','.',',').'</b></br>';
                    }
                    $ce = $checkValue+$ccc;
                    echo '<b>Next Monthly Installment: '.$this->actionGetModeDueDate(@$booking, @$booking->monthly_start_date,strtolower(@$modes->mode),round($ce)).'</b>';
                	break;
                }
			}
			    ...
			    if($checkValue != 6){
                    if($netTotalCheck > 0){
	                	   if($checkValue!=0){
	                        	...
	                        	echo '<b>Paid Upto: '.date('M, Y',strtotime(date("Y-m-d", strtotime(date('Y-m-d',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "-1 months")))) . "+$checkValuee months")).'</b><br/>';
	                    }
	                }
	                if($fractionVal!=0){
	                	echo '<b>Previos Balance: '.number_format((1-$fractionVal) * number_format(($modes->amount/6),'2','.',''),'2','.','').'<b></br>';
	                }
	                $checkValuee = ($checkValue==1 || $checkValue==0)?'6':($checkValue*6)-1;
	                echo '<b>Due Half Yearly '.floor($this->getDateDiff(date('d M, Y'),date('d M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValuee months")))/6).'<b></br>';
	                ...
	                if($fractionVal!=0){
	                    $dueTotal = ((1-$fractionVal) * number_format(($modes->amount/6),'2','.','')) + ($ccc * number_format(($modes->amount/6),'2','.',''));
	                    echo '<b>Total: '.number_format($dueTotal,'2','.',',').'</b></br>';
	                }
                    $ce = (($checkValue==1 || $checkValue==0)?1:$checkValue)+$ccc;
                    echo '<b>Next Half Yearly: '.$this->actionGetModeDueDate(@$booking, date('Y-m-d',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "-1 months")),strtolower(@$modes->mode),round($ce)).'</b>';
			...
			if($modes->mode=='monthly'){            
                if($checkValue != 36){ 
                	if($netTotalCheck > 0){
                            if($checkValue!=0){
                                $checkValue = $checkValue-1;
                                //echo '&nbsp;&nbsp;<br/><b>Paid Upto: '.date('M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValue months")).'</p></b>';
                            }}
                    if($fractionVal!=0){
                    		//echo number_format((1-$fractionVal) * number_format(($modes->amount/36),'2','.',''),'2','.','');
                    }
                    $checkValueMonthly = ($fractionVal!=0)?$checkValue+1:$checkValue;
                    //echo $this->getDateDiff(date('d M, Y'),date('d M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValueMonthly months")));
                    $ccc = $this->getDateDiff(date('d M, Y'),date('d M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValueMonthly months")));
                    //echo number_format(($modes->amount/36),'2','.','')." x $ccc  ";
                    echo number_format($ccc * number_format(($modes->amount/36),'2','.',''),'2','.',',');
                    if($fractionVal!=0){
                        $dueTotal = ((1-$fractionVal) * number_format(($modes->amount/36),'2','.','')) + ($ccc * number_format(($modes->amount/36),'2','.',''));
                        echo number_format($dueTotal,'2','.',',');
                    
                    }
                    
                        $ce = $checkValue+$ccc;
                        echo $this->actionGetModeDueDate(@$booking, @$booking->monthly_start_date,strtolower(@$modes->mode),round($ce));
                
                }
            } elseif($modes->mode=='yearly'){
                            if($checkValue != 6){
                                //echo date('M, o',strtotime($booking->monthly_start_date));
                                if($netTotalCheck > 0){
                                	   if($checkValue!=0){
                                    	    $checkValuee = ($checkValue==1)?6:($checkValue*6)-1;
                                ...
                                if($fractionVal!=0){
                                	//echo date('M, Y',strtotime(date("Y-m-d", strtotime(date('Y-m-d',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "-1 months")))) . "+$checkValue+6 months"));
                                    //echo number_format((1-$fractionVal) * number_format(($modes->amount/6),'2','.',''),'2','.','');
                                }
                                $checkValuee = ($checkValue==1 || $checkValue==0)?'6':($checkValue*6)-1;
                                //echo floor($this->getDateDiff(date('d M, Y'),date('d M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValuee months")))/6);
                                $checkValuee = ($checkValue==1 || $checkValue==0)?'6':($checkValue*6)-1;
                                $ccc = floor($this->getDateDiff(date('d M, Y'),date('d M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValuee months")))/6);
                                    //echo number_format(($modes->amount/6),'2','.','')." x $ccc  ";
                                    echo number_format($ccc * number_format(($modes->amount/6),'2','.',''),'2','.',',');
                                if($fractionVal!=0){
                                    $dueTotal = ((1-$fractionVal) * number_format(($modes->amount/6),'2','.','')) + ($ccc * number_format(($modes->amount/6),'2','.',''));
                                    echo number_format($dueTotal,'2','.',',');
                                }
                                    $ce = (($checkValue==1 || $checkValue==0)?1:$checkValue)+$ccc;
                                    echo $this->actionGetModeDueDate(@$booking, date('Y-m-d',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "-1 months")),strtolower(@$modes->mode),round($ce));
                                }
                        } else{
                        	echo $this->actionGetModeDueDate(@$booking, @$booking->createdOn,strtolower(@$modes->mode),round($checkValue)).'--->'.$modes->amount;
                        }
				*/
                echo '<br/>';
		endforeach;
	...
	public function actionGetLedgerDetail(){
		...
		foreach($cp as $c){
			echo 'Booking ID ---------------------->'.$c->id.'<br/>';
			$this->getPlotLedgerDetail($c->id);
			echo '<br/>';
		}
	}
