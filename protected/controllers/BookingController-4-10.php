...
class BookingController extends Controller
{
   ...
   //          }
			...
			if($modes->mode=='yearly'){
			    ...
			    if($checkValue != 6){
	                ...
	                if($fractionVal!=0){
	                	...
	                	$dueTotal = ($ccc * number_format(($modes->amount/6),'2','.',''));
	                }
	...
	public function getPlotLedgerDetailSingle($id,$mode,$csv=false){
		$booking = CustomerPlots::model()->findByPk($id);
		$paymentmodes = PaymentSchedulePaymentModes::model()->findAll('payment_schedule_id = :id AND plot_type = :type',array(':id'=>$booking->paymentSchedule->id,':type'=>strtolower($booking->plot->plot_type)));
	...
	public function actioncsvReport(){
		if($_POST['booking_id_csv']){
			...
			$list = array (
		        array('Block #','Plot Type','Plot #','Customer Name','Mobile Number','Booking Date','Installment Start Date','Booking','Confirmation','Allocation','Paid Month','Due Months','Monthly','Yearly',"Dealer",'Sub Dealer'),
		    );
		    $count = 1;
			foreach($bookingIds as $bookingId){
				$splitt = explode('*',$bookingId);
	...
	*/
	...
	public function actionBookingWarningLetter($id,$amount=0,$date=0,$monthly=0){
		...
		if($booking){
			...
			$wmCount = CustomerPlotsWarningLetters::model()->count('plot_id=:id',array(':id'=>$booking->id));
			$wlNumber = $this->getWarningLetterNo(@$booking->id);
			$wm = new CustomerPlotsWarningLetters;
			$wm->plot_id = $booking->id;
			$wm->warning_letter = $wmCount+1;
