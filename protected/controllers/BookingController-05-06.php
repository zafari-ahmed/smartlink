...
class BookingController extends Controller
{
   ...
   //          }
			$count++;
        endforeach;
        $fileName = 'AllBookings-'.date('Y-m-d-H-i').'.csv';
		$fp = fopen($fileName, 'w');
	...
	public function getPlotLedgerDetail($id){
		...
		//$checkMonth = $this->getAllMonths('2022-01-01');
		//echo '<pre>';print_r($checkMonth);exit;
		$booking = CustomerPlots::model()->findByPk($id);
		$paymentmodes = PaymentSchedulePaymentModes::model()->findAll('payment_schedule_id = :id AND plot_type = :type',array(':id'=>$booking->paymentSchedule->id,':type'=>strtolower($booking->plot->plot_type)));
		...
		foreach($paymentmodes as $modes): $rowBalance = 0;
			...
			if($modes->mode=='booking'){
				if($modes->amount != $var_sum->total){
			    	...
			    	$dddd = $this->actionGetModeDueDate(@$booking, @$booking->createdOn,strtolower(@$modes->mode),round($checkValue));
						...
						$html = '<p>Booking</p><br/><b>Total: '.number_format(floor($divideVal),'2','.',',').'</b></br>';
			...
			if($modes->mode=='confirmation'){
				if($modes->amount != $var_sum->total){
			    	...
			    	$dddd = $this->actionGetModeDueDate(@$booking, @$booking->createdOn,strtolower(@$modes->mode),round($checkValue));
				  //   	if (strpos($dddd, $checkMonth[0]) !== false && strpos($dddd, $checkMonth[1]) !== false) { 
						// 	$html .= '<b>Total: '.number_format(floor($divideVal),'2','.',',').'</b></br>';	
						// } else{ 
						// 	$html .= '0';
						// }
			    	$html = '<p>Confirmation</p><br/><b>Total: '.number_format(floor($divideVal),'2','.',',').'</b></br>';	
			    	$finalTotal = number_format(floor($divideVal),'2','.','');
					break;
				}
			}
			if($modes->mode=='allocation'){
				if($modes->amount != $var_sum->total){
				  ...
				  //   	if (strpos($dddd, $checkMonth[0]) !== false && strpos($dddd, $checkMonth[1]) !== false) { 
						...
						// 	$html .= '0';
						// }
			    	$html = '<p>Allocation</p><br/><b>Total: '.number_format(floor($divideVal),'2','.',',').'</b></br>';	
			    	$finalTotal = number_format(floor($divideVal),'2','.','');
			    	break;
				}
			}
			...
			if($modes->mode=='yearly'){
			    ...
			    if($checkValue != 6){
				    ...
				    if (strpos($dddd, $checkMonth[0]) !== false && strpos($dddd, $checkMonth[1]) !== false) { 
						if(@$dueTotal){
							$html = '<p>Half Yearly</p><br/><b>Total: '.number_format($dueTotal,'2','.',',').'</b></br>';
							$finalTotal = number_format($dueTotal,'2','.','');	
						} else{
							$html = '0';
							$finalTotal = 0;
	...
	public function getPlotLedgerDetailSingle($id,$mode){
		$booking = CustomerPlots::model()->findByPk($id);
		$paymentmodes = PaymentSchedulePaymentModes::model()->findAll('payment_schedule_id = :id AND plot_type = :type',array(':id'=>$booking->paymentSchedule->id,':type'=>strtolower($booking->plot->plot_type)));
		...
		foreach($paymentmodes as $modes): $rowBalance = 0;
			    ...
			    if($checkValue != 36){ 
                	if($netTotalCheck > 0){
                        if($checkValue!=0){
                        	...
                        	$html .= '<b>Paid Upto: '.date('M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+$checkValue+1 months")).'</b><br/>';
                        }
                    }
                    // if($fractionVal!=0){
                    // 		$html .= '<b>Previos Balance: '.number_format((1-$fractionVal) * number_format(($modes->amount/36),'2','.',''),'2','.','').'<b></br>';
                    ...
                    // }
                    // $ce = $checkValue+$ccc;
                    // $html .= '<b>Next Monthly Installment: '.$this->actionGetModeDueDate(@$booking, @$booking->monthly_start_date,strtolower(@$modes->mode),round($ce)).'</b>';
                	break;
                }
			}
			if($modes->mode=='yearly' && $modes->mode == $mode){
			    ...
			    if($checkValue != 6){
                    ...
                    $ce = (($checkValue==1 || $checkValue==0)?1:$checkValue)+$ccc;
                    $html .= '<b>Next Half Yearly: '.$this->actionGetModeDueDate(@$booking, date('Y-m-d',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "-1 months")),strtolower(@$modes->mode),round($ce)).'</b>';
	...
	public function actionGetLedgerDetail(){
		...
		$cp = CustomerPlots::model()->findAll('status = 1 AND blocked = 0');
		$result = [];
		//echo '<pre>';
		foreach($cp as $c){
	...
	public function getAllMonths($startDate){
		...
		foreach ($period as $dt) {
		    ...
		    $result['years'][] = $dt->format("Y");
		    $result['months'][] = $dt->format("M");
		}
	...
	public function actionsendupcomingmessage(){
		...
		foreach($result as $res){
			if($res['amount']!=0){
				$msg = "Dear ".ucfirst($res['customer']['name']).", \nWe’re pleased to inform you that the Plot ".$res['plot']['plot_type'].'-'.$res['plot']['plot_number']." in Block ".$res['plot']['block_number'].", has been booked.\nPlease receive your file from ESSA Housing Head office after 3 working days.\n\nThankyou for choosing Kainat city, for your dream land.\n\nFor any queries and further assistance please call at 021-37440935";
				echo $msg;exit;	
			}
	...
	*/
	...
	public function getTransactionMessage($id){
		$phaseId = Yii::app()->session->get('userModel')['phase_id'];
