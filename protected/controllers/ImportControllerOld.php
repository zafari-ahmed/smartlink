        ...
        //unset($result[0]);
            foreach ($result as $ind => $value) {
            	...
            	if(!empty($value[0])){
            		...
            		$agent = Agents::model()->find('name = :name AND parent_id IS NULL',array(':name'=>$value[8]));
						...
						if($agent){
							if($value[9]){
								...
								if(!$subAgent){
									...
									$subAgent->percentage_value = $value[12];
									$subAgent->number = '0';
									$subAgent->phase_id = 1;
									$subAgent->status = 1;
									$subAgent->save(false);
								...
								$parentIDD = $agent->id;
								$subAgent = Agents::model()->find('name = :name AND parent_id = :parent',array(':parent'=>$parentIDD,':name'=>$value[8]));
								//$subAgent = (object)$agent->attributes;	
							}
