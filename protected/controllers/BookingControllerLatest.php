...
class BookingController extends Controller
{
   ...
   //          }
	...
	public function getPlotLedgerDetail($id){
		$checkMonth = explode('-',date('M-Y',strtotime(date('Y-m-d')."+1 month")));
	...
	public function actionGetLedgerDetail(){
		...
		//$cp = CustomerPlots::model()->findAll('id = 5 AND status = 1');
		...
		foreach($cp as $c){
			...
			$result[$c->id]['ledger'] = $this->getPlotLedgerDetail($c->id);
			//print_r($result);exit;
		}
		$data['result'] = $result;
